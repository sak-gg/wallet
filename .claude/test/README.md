...
- [handler_tests.md](handler_tests.md) — HTTP layer (`internal/handler/wallet_handler_test.go`), 12 cases
- [service_tests.md](service_tests.md) — business logic (`internal/service/wallet_service_test.go`), 13 cases
- [repository_concurrency_tests.md](repository_concurrency_tests.md) — real-MySQL concurrency/integration (`internal/repository/mysql/wallet_repository_concurrency_test.go`), 2 cases

**Total: 27 test cases** across 3 files, plus one test-support fake (`internal/service/fake_repository_test.go`, an in-memory repository double — not a test itself).
